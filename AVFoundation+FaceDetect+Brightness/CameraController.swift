...
extension CameraController: AVCaptureMetadataOutputObjectsDelegate {
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        for object in metadataObjects {
            guard let transformedObject = previewLayer?.transformedMetadataObject(for: object) else {
                continue
            }
            print(transformedObject.bounds)
        }
    }
}
...
extension CameraController: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if let metadata = metadataDict as? [AnyHashable : Any],
            ...
            let brightness = exifMetadata[kCGImagePropertyExifBrightnessValue as String] as? Double {
            if brightness < 0 {
                print("太暗了")
            }
        }
